...
예를 들어, setImageViewBitmap(), setTextViewText() 같은 메서드는 내부적으로 리플렉션 용도의 메서드인 setBitmap(int viewId, String methodName, Bitmap value), setCharSequence(int viewId, String methodName, CharSequence value)를 다시 호출한다(``setImageBitmap'', ``setText'' 문자열이 methodName에 들어간다).\\
