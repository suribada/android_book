...
\item 28라인(4)에서 ACTION\_APPWIDGET\_ENABLED Action은 최초 설치 시에 호출된다.
...
그런데 캘린더나 시간표 같은 몇 가지 앱은 사용자의 요구로 인해 App Widget의 사이즈가 4x4나 5x5 정도까지 해서 단순 정보가 아닌 화면을 꽉 채운 정보를 전달하는 경우가 있다.
...
기존에는 문제가 없었는데 특히 최신 단말에서 App Widget 생성하면서 Bitmap.createBitmap()에서 OutOfMemoryError가 발생한다면 이것을 원인으로 보면 거의 맞다.
