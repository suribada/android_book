...
ACTION\_MEDIA\_SC\-A\-NNER\_SC\-AN\_FI\-LE Action 같은 것은 앱에서 발생시키라고 있는 것이다.
...
RemoteViews의 레이아웃에서 쓸 수 있는 뷰 클래스는 한정되어 있는데, LayoutInflator.Filter 인터페이스의  onLoadClass() 메서드가 뷰 클래스를 제한하는 역할을 한다. LayoutInflator.Filter 구현체인 RemoteViews에서 boolean onLoadClass(Class clazz) 메서드를 보면 클래스 선언에 @RemoteView 애노테이션이 있는 것만 true를 리턴한다.
...
setImageViewBitmap(), setImageViewUrl(), setTextColor(), setTextViewText() 같은 메서드는 모두 내부적으로 이 리플렉션 용도의 메서드인 setBitmap(int viewId, String methodName, Bitmap value),  setUri(int viewId, String methodName, Uri value), setInt(int viewId, String methodName, int value), setCharSequence(int viewId, String methodName, CharSequence value)를 다시 호출한다(``setImageBitmap'', ``setImageURI'', ``setTextColor'', ``setText'' 문자열이 methodName에 들어간다).\\
