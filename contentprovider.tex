...
마음 같아서는 다시 작성해서 동일한 버전으로 하면 코드도 깔끔할 것 같은데, 기존에 이 버전을 사용중인 단말은 DB 버전이 동일하므로 onUgrade()는 실행되지 않고 변경된 테이블을 사용할 수 없기 때문에 그래서는 안된다.
버전을 올리고서 onUprade()에서 테이블 구조를 변경하는 코드가 들어가는 게 정상적인 방법이다.
