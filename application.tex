...
\begin{itemize}
\item onConfigurationChanged(Configuration newConfig): Activity 외에 다른 컴포넌트는 Configuration이 변경되어도 재시작하지 않고 onConfigurationChanged() 메서드가 불린다. 다른 컴포넌트보다 Application의  onConfigurationChanged()가 먼저 실행되는 것을 볼 수 있다.
...
앱에는 ShoppingApplication과 CategoryUpdaterService(디폴트 프로세스)가 있고, ActivityA(메인 Activity, 디폴트 프로세스), ActivityB(and\-roid:process=``:ca\-mera'')가 있다고 하자. 
