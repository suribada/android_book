...
\item 분리된 프로세스에서는 SharedPreferences를 쓰는 것은 권장되지 않는다. 
SharedPreferences를 어차피 파일이기 때문에 한번 읽어오는 것은 문제가 되지 않는다. 그런데, 다른 프로세스에서 값을 변경한 것을 다시 가져오는 것은 문제가 간단하지 않다.
허니콤 이후에는 
...
그런데 마시멜로에서는 다시 Context.MODE\_MULTI\_PROCESS를 depecation시켰다. 결과적으로 권장되는 방식은 SharedPreferences를 다시 ContentProvider로 감싸서 다른 프로세스에서 접근하게 하는 것이다.
