...
예를 들어, 어떤 메서드가 단순하게 TextView의 setText()를 실행하는 것이지만, 메서드 호출을 하는 곳이 여러 군데이거나 메서드 호출 스택이 깊어서 어디서 호출하는지 금방 파악이 안 되는 경우도 있다.
...
권한 문제나 심각한 서버 문제처럼, 앱을 더 이상 사용할 수 없는 특별한 때가 아니면 사용할 일이 없다. 남용하면 안 되는 메서드이다.
\end{itemize}
...
MessageQueue.IdleHandler API 문서를 보면, IdleHandler 인터페이스를 가지고서 Message를 모두 처리하고서 휴식 상태일 때 할 일을 지정할 수 있다.
