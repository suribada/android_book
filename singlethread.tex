...
메인 스레드라면 이미 메인 Looper가 있어서 문제가 되지 않지만, 백그라운드 스레드라면 대응하는 Looper가 당장 없을 때 RuntimeException을 만나게 된다.\\
...
안드로이드 버전이 올라가면서 많은 논란 가운데 하나가 AsyncTask를 실행할 때 기본 동작이 `병렬 실행'에서 `순차 실행'으로 바뀐 것이다. 
