...
\begin{lstlisting}[frame=single, caption=서비스에서 스레드 실행, label=SleepService]
...
Service는 보통 Started Service 또는 Bound Service로 존재하는데, Started이면서 Bound일 수도 있다. 
...
여러 클라이언트에서 startService()를 실행한다면 각각 작업이 끝났을 때 아직 진행 중인 것이 있는지 알 수 없는 경우도 있다. 
...
IntentService의 구조는 단순하다.  HandlerThread를 onCreate()에서 생성해서 시작하면서 HandlerThread의 Looper와 연결된 Handler를 만들다.
...
이제 onCreate()가 아닌 onStartCommand() 메서드에서 이런 skip 방식을 적용한 샘플을 보도록 하자.
\begin{lstlisting}[frame=single]
...
여기서 사용한 Executors.newSingleThreadExecutor() 같은 정적 메서드는 많이 사용되는 TheadPoolExecutor를 팩토리 메서드로 만든 것이다. 
이번에는 ThreadPoolExecutor를 직접 사용해서 skip을 적용한 샘플을 만들어보자.
\begin{lstlisting}[frame=single]
public class SleepThreadService extends Service {
...
아래 코드는 포매팅을 한 것이고 실제로는 탭이 없이 빽빽하게 붙어있다. 이 소스는 들여다볼 일이 많지 않지만 한 번쯤은 봐두는 게 좋다.
\begin{lstlisting}[frame=single]
...
그런데 여기서 만든 로컬 바인딩 샘플을 보면 뭔가 억지로 끼워 맞춘 느낌이 있다. 
Service의 메서드를 직접 호출할 수 없으니 바인딩을 통해서 Service 레퍼런스를 가져오고 그 다음은 Service 메서드를 직접 호출하는 것에 불과하다.
